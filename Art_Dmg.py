...
Resistance = tk.IntVar(value=0)               # Resistance 
...
def OP_ATK():
    ...
    if ent_Resistance.get().isdigit():
        Resistance = int(ent_Resistance.get())
    else:
        print("Resistance is not a number")
...
def Physical_DMG_Formula():
    ...
    Phy_DMG_Taken()
    OP_ATK()
    global Resistance, Flat_Def_Down, Physical_DMG_Taken, Scaling_Def_Down
   
    D = (Resistance + Flat_Def_Down)
    if D<0: #To check Resistance don't go below 0
        D=0
    result = Final_Attack() *(1-(D)*(1-Scaling_Def_Down)/100)* Physical_DMG_Taken
    print("-"*30)
    print("D is ", D)
    ...
    lbl_Def_FlatDebuff.config(text=f'Flat Resistance Debuff: {Flat_Def_Down} ')
    lbl_Def_Scal.config(text=f'Scaling Resistance Debuff: {Scaling_Def_Down:.3f} ')
...
#Resistance 
lbl_Resistance = ttk.Label(frm_Stats, text= "Enemy Resistance: ")
ent_Resistance = ttk.Entry(frm_Stats)
lbl_Resistance.grid(row=1, column=0, padx=10, pady=10)
ent_Resistance.grid(row=1, column=1, padx=10, pady=10)
ent_Resistance.insert(0,'0')
...
def add_debuff():
    ...
    lab = ttk.Label(frm_DeBuffs, text=f"Flat Resistance deBuff {next_row+1}")
    lab.grid(row=next_row+1, column=0, padx=5, pady=10)
...
def add_debuffp():
    ...
    lab = ttk.Label(frm_DeBuffs, text=f"Percentage Resistance deBuff {next_row+1}")
    lab.grid(row=next_row+1, column=2, padx=5, pady=10)
    
    ent_debuffsp = ttk.Entry(frm_DeBuffs)
    ent_debuffsp.grid(row=next_row+1, column=3, padx=1, pady=10)
...
frm_DeBuffs.place(x=570,y=10)



btn_more_debuff = ttk.Button(frm_DeBuffs, text='Add Flat Resistance Debuff',command=add_debuff)
btn_more_debuff.grid(row=0, column=0, padx=10, pady=10)

btn_more_debuffp = ttk.Button(frm_DeBuffs, text='Add Percentage Resistance Debuff', command=add_debuffp)
...
lbl_Tab1 = ttk.Label(notebookTab1,
                     ...
                     wraplength=318,font=("Arial", 15), compound=tk.TOP)
...
lbl_Tab3 = ttk.Label(notebookTab3,
                     text="⩺Debuffs Frame:\n ⤿Flat Resistance Debuff is any debuff with flat number (not %) Example ( -🛡 DEF): Ifrit s2 and Meteorite s2.\n\n⤿Percentage Resistance Debuff is any debuff with percentage (-🛡% DEF) Example: Pramanix s2 and Schwarz Talent.",
                     wraplength=320,font=("Arial", 16), compound=tk.TOP)
...
lbl_Def_FlatDebuff = ttk.Label(frm_result, text="Flat Resistance Debuff: --- ")
lbl_Def_FlatDebuff.grid(row=0, column=5, padx=5, pady=10)

lbl_Def_Scal = ttk.Label(frm_result, text="Scaling Resistance Debuff: --- ")
